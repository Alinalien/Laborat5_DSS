...
public class Fight {
    ...
    public void EndRound(Player human, Player enemy, JDialog dialog,
        ...
        if (human.getHealth() > 0) {
            ...
            label.setText(enemy.getName() + " wins");
        }
         ...
         // Вбиваю количество локаций для следующего раунда
         SetNRoundsLocation(human.getLevel());
         ((Human)human).setZeroRound();

         // Показываю, что локация изменена
    ...
    public Player NewRound(Player human, JLabel label, JProgressBar pr1,
        ...
        Player enemy1 = null;
        if (((Human) human).getNRound() == this.NRoundsLoc-1) { 
